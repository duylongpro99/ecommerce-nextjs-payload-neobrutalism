import { ExtendedDriveFile } from '@/lib/google-drive';
...
export const DriveFileViewer: React.FC<DriveFileViewerProps> = ({ fileData }) => {
    const renderFileViewer = () => {
        ...
        if (mimeType.startsWith('image/')) {
            return <img src={directLink} alt={name} className="max-w-full h-auto rounded-lg shadow-lg" style={{ maxHeight: '65vh' }} />;
        }

        if (mimeType === 'application/pdf') {
            return <iframe src={embedLink} className="w-full rounded-lg shadow-lg" style={{ height: '65vh' }} title={name} />;
        }

        if (mimeType.startsWith('video/')) {
            return (
                <video controls className="max-w-full h-auto rounded-lg shadow-lg" style={{ maxHeight: '65vh' }}>
