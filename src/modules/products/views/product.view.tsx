                    ...
                    <div className="col-span-4">
                        ...
                        <div className="border-y flex">
                            ...
                            <div className="px-6 py-4 flex items-center justify-center lg:border-r">
                                <Link href={generateTenantUrl(tenantSlug)} className="flex items-center gap-2" prefetch>
                                    ...
                                    <p className="font-medium tesxt-base underline">{data.tenant?.name}</p>
                                </Link>
                            </div>

                            <div className="hidden lg:flex px-6 py-4 items-center justify-center">
                                <div className="flex items-center gap-2">
                                    <StarRating rating={data.reviewRatings} iconClassName="size-4" />
                                    <p className="text-base font-medium">{data.reviewCount} rating(s)</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="border-t lg:border-t-0 lg:border-l h-full">
                            <div className="flex flex-col gap-4 p-6 border-b">
                                <div className="flex flex-row items-center gap-2">
                                    <CartButton productId={id} tenantSlug={tenantSlug} isPurchased={data.isPurchased} />
                                    <Button
                                        variant={'elevated'}
                                        className="size-12"
                                        onClick={() => {
                                ...
                                <p className="text-center font-medium">
                                    {data.refundPolicy === 'no-refund' ? 'No refund' : `${data.refundPolicy} money back guarantee`}
                                </p>
                            </div>
                                    ...
                                    {[5, 4, 3, 2, 1].map((stars) => {
                                        return (
                                            <Fragment key={`start-${stars}`}>
                                                ...
                                                <Progress value={data.ratingDistributions[stars]} className="h-[1lh]" />
                                                <div className="font-medium">{data.ratingDistributions[stars]}%</div>
                                            </Fragment>
                                        );
                                    })}
                                </div>
